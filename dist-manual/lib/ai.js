...
const DialogGPTService = require('./dialogGPTService');

class AIService {
  constructor() {
    this.mode = 'dialogpt'; // 'dialogpt' or 'gemini'
    this.genAI = null;
    this.model = null;
    this.dialogGPT = null;
  ...
  async initialize(apiKey = null, mode = 'dialogpt') {
    this.mode = mode;
    
    if (mode === 'dialogpt') {
      console.log('🤖 Initializing BlueChip DialoGPT Service...');
      this.dialogGPT = new DialogGPTService();
      await this.dialogGPT.initialize();
      this.isInitialized = true;
      console.log('✅ DialoGPT Service ready');
  ...
  async chat(message, context = '') {
    ...
    if (this.mode === 'dialogpt') {
      return await this.dialogGPT.chat(message, context);
    } else {
      return await this.geminiChat(message, context);
    }
  }
...
Keep responses concise and conversational. Don't over-format simple responses.`;
  ...
  async summarize(url, title) {
    ...
    if (this.mode === 'dialogpt') {
      return await this.dialogGPT.chat(`Summarize this: ${title} - ${url}`);
    } else {
      return await this.geminiSummarize(url, title);
    }
  }

  async geminiSummarize(url, title) {
    if (!this.isInitialized) {
      throw new Error('AI service not initialized. Please provide an API key.');
    }
